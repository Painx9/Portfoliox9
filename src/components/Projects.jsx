...
const Projects = () => {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      // Run GSAP 3D animation primarily on screens with enough width
      if (window.innerWidth >= 768) {
        if (!selectedRepo) {
          ...
          folderCardsRef.current.forEach((card) => {
            if (!card) return;
            gsap.set(card, { xPercent: -50, yPercent: -50, rotation: gsap.utils.random(-6, 6), scale: 0.85, x: 0, y: 0 });
          });
          ...
          tl.to(folderFrontRef.current, { rotationX: -130, duration: 1.2, ease: "power3.inOut" })
            .to(folderCardsRef.current, { y: -120, scale: 0.9, duration: 0.6, stagger: 0.08, ease: "back.out(1.2)" }, "-=0.6")
            .to(folderCardsRef.current, {
              x: (i) => (i - 2) * 270 + gsap.utils.random(-20, 20),
              y: (i) => (i % 2 === 0 ? -20 : 20) + gsap.utils.random(-10, 10),
          ...
          subTl.to(subFolderFrontRef.current, { rotationX: -130, duration: 1.0, ease: "power3.inOut" })
            .to(subFolderCardsRef.current, { y: -100, scale: 0.9, duration: 0.5, stagger: 0.06, ease: "back.out(1.2)" }, "-=0.5")
            .to(subFolderCardsRef.current, {
              x: (i) => {
                const count = selectedRepo.subProjects.length;
                return (i - (count - 1) / 2) * 300 + gsap.utils.random(-20, 20);
              },
              y: (i) => (i % 2 === 0 ? -15 : 15) + gsap.utils.random(-10, 10),
              rotation: () => gsap.utils.random(-4, 4),
              scale: 1,
              duration: 1.0,
              stagger: 0.08,
  ...
  return (
    <section id="projects" ref={containerRef} className="bg-[#0b0b0b] min-h-screen relative font-sans text-white w-full flex flex-col items-center justify-center py-20 px-4 md:px-12 select-none overflow-x-hidden">
      
      {/* Background Watermark Heading */}
      <div className="absolute top-10 left-0 w-full flex items-start justify-center pointer-events-none z-0">
        <h1 className="text-[12vw] sm:text-[16vw] font-black text-white/[0.03] tracking-tighter leading-none whitespace-nowrap uppercase">
      ...
      {/* Breadcrumb Header */}
      <div className="relative z-30 w-full max-w-7xl mb-6 flex flex-col items-center justify-center text-center gap-2">
        {selectedRepo ? (
      ...
      {/* ========================================================= */}
      {/* MOBILE LAYOUT (Stacked clean grid for phones & small screens) */}
      {/* ========================================================= */}
      <div className="md:hidden w-full max-w-sm z-20 space-y-4 my-4">
        {!selectedRepo ? (
          repositoryGroups.map((repo, i) => (
            <div 
              key={i} 
              onClick={() => setSelectedRepo(repo)}
              className="w-full bg-[#141414]/95 border border-white/15 rounded-2xl p-5 shadow-xl active:border-red-600 cursor-pointer flex flex-col justify-between space-y-3"
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono font-bold text-red-500 bg-red-600/10 px-2 py-0.5 rounded border border-red-600/20">{repo.episode}</span>
                <span className="text-xs font-mono text-red-400 font-bold">Open &rarr;</span>
              </div>
              <div>
                <h3 className="text-lg font-black text-white">{repo.repoName}</h3>
                <p className="text-xs text-white/70 font-light mt-1">{repo.description}</p>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-white/10 text-[10px] font-mono text-white/50">
                <span>{repo.subProjects.length} Modules</span>
                <span className="text-red-500 font-bold">[ TAP TO VIEW ]</span>
              </div>
            </div>
          ))
        ) : (
          selectedRepo.subProjects.map((project, idx) => (
            <div 
              key={idx}
              className="w-full bg-[#141414]/95 border border-white/15 rounded-2xl p-5 shadow-xl space-y-3"
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono font-bold text-red-500 bg-red-600/10 px-2 py-0.5 rounded">MODULE 0{idx + 1}</span>
              </div>
              <div>
                <h3 className="text-base font-black text-white">{project.title}</h3>
                <p className="text-xs text-white/80 font-light mt-1">{project.mission}</p>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-white/10">
                <a href={project.repoLink} target="_blank" rel="noopener noreferrer" className="flex-1 py-2 bg-white/10 text-white font-mono text-[10px] font-bold uppercase rounded text-center border border-white/15">Code</a>
                <a href={project.appLink} target="_blank" rel="noopener noreferrer" className="flex-1 py-2 bg-red-600 text-white font-mono text-[10px] font-bold uppercase rounded text-center shadow-[0_0_10px_rgba(229,9,20,0.5)]">Launch</a>
              </div>
            </div>
          ))
        )}
      ...
      {/* ========================================================= */}
      {/* DESKTOP LAYOUT (Cinematic 3D GSAP Stack for tablets & PCs) */}
      {/* ========================================================= */}
      <div className="hidden md:flex relative w-full max-w-7xl h-[480px] items-center justify-center perspective-[2000px] z-10">
        <div className="relative w-0 h-0 transform-style-3d overflow-visible">
          
          {!selectedRepo ? (
            <>
              <div ref={folderBackRef} className="absolute w-[32vw] max-w-[380px] aspect-[4/3] bg-[#141414] rounded-[24px] border border-red-600/40 shadow-[0_20px_50px_rgba(229,9,20,0.25)] flex items-center justify-center" style={{ zIndex: 5 }}>
              ...
              {repositoryGroups.map((repo, i) => (
                <div key={i} ref={el => folderCardsRef.current[i] = el} onClick={() => setSelectedRepo(repo)} className="absolute w-[30vw] max-w-[330px] aspect-[4/4.2] will-change-transform cursor-pointer hover:!z-[999] group/card" style={{ zIndex: 10 + i }}>
              ...
              <div ref={folderFrontRef} className="absolute w-[32vw] max-w-[380px] aspect-[4/3] pointer-events-none will-change-transform" style={{ zIndex: 60 }}>
            ...
            <>
              <div ref={subFolderBackRef} className="absolute w-[32vw] max-w-[380px] aspect-[4/3] bg-[#141414] rounded-[24px] border border-red-600/40 shadow-[0_20px_50px_rgba(229,9,20,0.25)] flex items-center justify-center" style={{ zIndex: 5 }}>
                <div className="absolute -top-6 left-6 w-32 h-8 bg-[#1f1f1f] rounded-t-xl border-t border-red-600/30" />
              ...
              {selectedRepo.subProjects.map((project, idx) => (
                <div key={idx} ref={el => subFolderCardsRef.current[idx] = el} className="absolute w-[30vw] max-w-[330px] aspect-[4/4.2] will-change-transform cursor-pointer hover:!z-[999] group/subcard" style={{ zIndex: 10 + idx }}>
              ...
              <div ref={subFolderFrontRef} className="absolute w-[32vw] max-w-[380px] aspect-[4/3] pointer-events-none will-change-transform" style={{ zIndex: 60 }}>
                <div className="absolute bottom-0 w-full h-[85%] bg-[#1c1c1c] rounded-b-[24px] rounded-t-md shadow-[0_-5px_20px_rgba(0,0,0,0.8)] flex flex-col justify-end p-6 border-t border-red-600/40">
                  <div className="w-20 h-1.5 bg-white/20 rounded-full mx-auto mb-2" />
                </div>
              </div>
